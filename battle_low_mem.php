<?
	...
	function unit_set($type,$unit_string,$value) {
		//str_pad($ui,3,"0",STR_PAD_LEFT).str_pad($s,6,"0",STR_PAD_LEFT).str_pad($h,8,"0",STR_PAD_LEFT);
		switch($type) {
			case "u":
	...
	function up($unit,$id,$team) {
		...
		$unit_s = unit_get("s",$unit);
		$unit_h = unit_get("h",$unit);
		echo($is[$unit_u]."($team:$id) ".$attack[$is[$unit_u]]*(1+$_POST["$team"."_a"]/10).":$unit_s:$unit_h");
	}
	...
	if($_POST['button3'] == "Simulate") {
		...
		if($debug) {
			...
			lp("<strong>starting simulation ".date("H:i:s",time())."</strong>");
		}
		//$wins = 0;
		$ave = array();
		$costs = array();
		...
		for($n=0;$n<$sims;$n++) {
			if($debug) {
				echo memory_get_usage();
				lp(" loading battle field");
				lp("round $n loading battle field ".date("H:i:s",time()));
			}
			set_time_limit(0);
			$battle = array();
			foreach($_POST as $key => $val) {
			...
			if($debug) {
				echo memory_get_usage();
				lp(" loaded battle field");
				//echo "<pre>";
				//print_r($battle);
				lp(" loaded battle field ".date("H:i:s",time())." ");
			}
			...
			for($i=0;$i<6;$i++) {
				if($debug) {
					lp($n." battle round $i ".date("H:i:s",time())." ");
				}
				...
				if($debug) {
					...
					lp($n." dealing damage ".date("H:i:s",time())." ");
				}
							...
							foreach(array_keys($us) as $uid) {
								...
								while($rapid_fire) {
									...
									if($enemy_s>=$damage) {
										...
										//echo " #$enemy $enemy_h - $hull_damage# ";

										
										if($debug_battle) {
											echo " suffers $enemy_s shield and $hull_damage hull damage ";
											//echo $enemy;
										}
				...
				if($debug) {
					...
					lp($n." destroying ships ".date("H:i:s",time())." ");
				}
