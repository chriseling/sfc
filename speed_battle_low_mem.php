<?
	...
	function unit_get($type,$unit_string) {
		switch($type) {
			case "u":
				return (int)substr($unit_string,0,3);
			case "s":
				return (int)substr($unit_string,3,6);			
			case "h":
				return (int)substr($unit_string,9,8);
		}
	}
	
	function unit_set($type,$unit_string,$value) {
		switch($type) {
			case "u":
				return str_pad($value,3,"0",STR_PAD_LEFT).substr($unit_string,3,14);
			case "s":
				return substr($unit_string,0,3).str_pad($value,6,"0",STR_PAD_LEFT).substr($unit_string,9,8);			
			case "h":
				return substr($unit_string,0,9).str_pad($value,8,"0",STR_PAD_LEFT);
		}
	}
	...
	function cost($team,$type,$resource) {
		...
		if(count($costs)) {
			if($type == "debris") {
				return number_format($costs[$team][$type][$resource]/100*30,0);
			} else {
				return number_format($costs[$team][$type][$resource],0);
			}
		} else {
			return "";
		}
		...
		if(!$_POST['them_h']) {
			...
			foreach($_POST as $key => $val) {
				if(preg_match("/(\w+)\_(\w+)/",$key,$matches) && $val) {
					...
					if(in_array($unit,$is)) {
						...
						for($i=0;$i<$val;$i++) {
							$battle[$team][] = str_pad($ui,3,"0",STR_PAD_LEFT).str_pad($s,6,"0",STR_PAD_LEFT).str_pad($h,8,"0",STR_PAD_LEFT);
						}
					}
				}
			}
			...
			for($i=0;$i<6;$i++) {
				if(count($battle["us"]) && count($battle["them"])) {
					foreach(array_keys($battle) as $team) {
							$us = $battle[$team];
							foreach(array_keys($us) as $uid) {
								$unit = $battle[$team][$uid];
								$unit_u = unit_get("u",$unit);
								$rapid_fire = true;
								while($rapid_fire) {
									$target = rand(0,count($battle[$opposite[$team]])-1);
									$enemy = &$battle[$opposite[$team]][$target];
									$enemy_s = unit_get("s",$enemy);
									$enemy_u = unit_get("u",$enemy);
									$enemy_h = unit_get("h",$enemy);
									$damage = $attack[$is[$unit_u]]*(1+$_POST["$team"."_a"]/10);									
									if($enemy_s>=$damage) {
										if($damage > $shield[$is[$enemy_u]]*(1+$_POST[$opposite[$team]."_s"]/10)*.01) {
											$enemy = unit_set("s",$enemy,$enemy_s - $damage);
										}
									} else {
										$hull_damage = $damage - $enemy_s;
										$enemy = unit_set("h",$enemy,$enemy_h - $hull_damage);
										$enemy = unit_set("s",$enemy,0);
									}
									$r = $rf[$is[$unit_u]][$is[$enemy_u]];
									if($r) {
				...
				if(count($battle["us"]) && count($battle["them"])) {
					foreach($battle as $team => $us) {
							foreach($us as $uid => $unit) {
								...
								$victim_s = unit_get("s",$victim);
								$unit_u = unit_get("u",$unit);
								$unit_h = unit_get("h",$unit);
								$ui = $is[$unit_u];
								if($unit_h > .7*$hull[$ui]*(1+$_POST["$team"."_h"]/10)) {
									$victim = unit_set("s",$victim,$shield[$ui]*(1+$_POST["$team"."_s"]/10));
								} else if ($unit_h > 0) {
									$grim_reaper = rand(1,100);
									$hull_remaining = ceil($unit_h/$hull[$ui]/(1+$_POST["$team"."_h"]/10)*100);
      ...
      <div style="background-color:#F99191; padding:15px;"><strong>THEM</strong>
	    <p>
	      <select name="saved_them" id="saved_them" onchange="submit()">
	        ...
	        <?
				$sql = "select * from saved where type = 'them' and who = '$user' order by id desc limit 50";
				$res = mysql_query($sql);
	    ...
	    <p>
	      <select name="saved_us" id="saved_us" onchange="submit()">
	        ...
	        <?
				$select = "";
				$sql = "select * from saved where type = 'us' and who = '$user' order by id desc limit 50";
				$res = mysql_query($sql);
				while($mr = mysql_fetch_assoc($res)) {
       ...
       <br />
        ...
        <table>
          ...
          <tr>
            ...
            <? 
				$plunder_percent = 0;
			   } else if($costs["us"]["cargo"]["o"] > $plunder_avail) { ?>
				<?=number_format($plunder_avail,0);?> / <?=number_format($plunder_avail,0);?> (100% plundered)
            <? 
				$plunder_percent = 1;
			} else { ?>
			<?=cost("us","cargo","o");?> / <?=number_format($plunder_avail,0);?>
 (<?=number_format($costs["us"]["cargo"]["o"] / $plunder_avail *100,1);?>% plundered - need <?=ceil(($plunder_avail-$costs["us"]["cargo"]["o"])/25000);?> hercs)
 				<? 
				
				$plunder_percent = $costs["us"]["cargo"]["o"] / $plunder_avail;
				} ?>
 			<? } ?>
 			</td>
          </tr>
          <tr>
            ...
            <td>			<? if(1) { ?> <?=cost("total","debris","oc");?> (<?=ceil($costs['total']['debris']['oc']/20000*.3);?> dios)<? } ?></td>
          </tr>
          <tr>
            ...
            <table><tr>
            <td>o:</td>
            <td><?=number_format($costs["total"]["debris"]["o"]*.3 + $plunder_percent*$filled['them_ore']/2 - $costs['us']['loss']['o'],0);?></td>
            <td>c:</td>
            <td><?=number_format($costs["total"]["debris"]["c"]*.3 + $plunder_percent*$filled['them_crystal']/2 - $costs['us']['loss']['c'],0);?></td>
            <td>h:</td>
            <td><?=number_format($plunder_percent*$filled['them_hydrogen']/2 - $costs['us']['loss']['h'],0);?></td>
          </tr></table>
